...
longest :: Ord a => [[a]] -> [a]
longest xss = snd $ foldr min (0, []) (ordering xss)
  where ordering = map (\xs -> (-length xs, xs))
------------------------------------------------------------------------------
...
-- winner scores player1 player2 = snd . snd $ max (ordering 1 player1) (ordering 2 player2)
--   where ordering pos player = (Map.findWithDefault 0 player scores, (-pos, player))
...
transfer from to amount bank = 
  if Map.member from bank && Map.member to bank && amount >= 0 && balance from bank >= amount
    then Map.insert to (balance to bank + amount) (Map.insert from (balance from bank - amount) bank)
    else bank
    where
      balance account bank = fromJust . Map.lookup account $ bank
      fromJust Nothing = -1
      fromJust (Just a) = a
------------------------------------------------------------------------------
...
-- maxIndex arr = fst $ foldr larger (head pairs) pairs
--   where larger x y = if snd x >= snd y then x else y
--         pairs = assocs arr
