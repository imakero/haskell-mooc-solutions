...
buyOneMore (MkShoppingEntry product price quantity) = MkShoppingEntry product price (quantity + 1)
...
toNat z
  | z < 0 = Nothing
  | z == 0 = Just Zero
  | otherwise = Just (foldr (\x acc -> PlusOne acc) Zero [1..z])
...
fromBin :: Bin -> Int
fromBin = helper 0 
  where helper k (I b) = 2^k + helper (k+1) b
        helper k (O b) = helper (k+1) b
        helper k End = 0

toBin :: Int -> Bin
-- toBin = helper
--   where helper 0 = End
--         helper n = inc $ helper (n-1)
toBin = helper
  where helper 0 = O End
        helper 1 = I End
        helper n = 
          if n `mod` 2 == 1 
            then I (helper $ div (n-1) 2)
            else O (helper $ div n 2)