...
firstHalf :: [a] -> [a]
firstHalf xs = take half_ceil xs
  where
    half_ceil = div len_x 2 + mod len_x 2
    len_x = length xs
...
--     go [] = []
--     go (c : cs) = case c of
--       "printY" -> show (count "up" cs - count "down" cs) : go cs
--       "printX" -> show (count "right" cs - count "left" cs) : go cs
--       _ -> go cs
--     count x [] = 0
