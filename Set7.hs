...
member _ (Set []) = False
member val (Set (x : xs)) = val == x || member val (Set xs)

-- add a member to a set
add :: Ord a => a -> Set a -> Set a
add val (Set xs) = Set (nub (insert val xs))
...
passwordAllowed p (ContainsSome cs) = not (null (intersect p cs))
passwordAllowed p (DoesNotContain cs) = null (intersect p cs)
...
data Arithmetic = Value Integer | Add Arithmetic Arithmetic | Multiply Arithmetic Arithmetic
  deriving (Show)
...
evaluate (Multiply i j) = evaluate i * evaluate j

render :: Arithmetic -> String
render (Value n) = show n
render (Add i j) = "(" ++ render i ++ "+" ++ render j ++ ")"
render (Multiply i j) = "(" ++ render i ++ "*" ++ render j ++ ")"
