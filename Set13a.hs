...
checkNumber (s, t)
  | noOverlap s digits && noOverlap t digits = Just (s, t)
  | otherwise = Nothing
  where
    noOverlap u v = null $ u `intersect` v
    digits = "0123456789"
...
checkCapitals (for, sur)
  | startsWithCapital for && startsWithCapital sur = Just (for, sur)
  | otherwise = Nothing
  where
    startsWithCapital (c : _) = isUpper c
...
selectSum :: (Num a, Show a) => [a] -> [Int] -> Maybe a
selectSum xs is = do
...
--   | c == '(' = do
--     old <- get
--     put (if old < 0 then old else old + 1)
--   | otherwise = do
--     old <- get
--     put (if old < 0 then old else old -1)
...
--     (Just _) -> put $ inc state
--     Nothing -> put $ state ++ [(x, 1)]
--   where
--     inc = map (\(b, n) -> if b == x then (b, n + 1) else (b, n))
