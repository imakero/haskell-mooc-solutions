...
statements as bs = glue <$> as <*> [" is ", " is not "] <*> bs
  where
    glue a b c = a ++ b ++ c
...
calculator op n = ($) <$> parsedOp <*> readMaybe n
  where
    parsedOp
      | op == "negate" = Just negate
      | op == "double" = Just (* 2)
      | otherwise = Nothing
...
validateAddress streetName streetNumber postCode =
  Address <$> checkStreet <*> checkNum <*> checkPc
  where
    checkStreet = check (length streetName <= 20) "Invalid street name" streetName
    checkNum = check (isDigits streetNumber) "Invalid street number" streetNumber
    checkPc = check (isDigits postCode && length postCode == 5) "Invalid postcode" postCode
    isDigits s = all isDigit s
...
boolOrInt x = check (isBool x) "Not a Bool" (Left $ parseBool x) <|> check (isInt x) "Not an Int" (Right $ parseInt x)
  where
    isBool a = a == "True" || a == "False"
    parseBool a
      | a == "True" = True
      | otherwise = False
    isInt a = case (readMaybe a :: Maybe Int) of
      Just _ -> True
      Nothing -> False
    parseInt a = case readMaybe a of Just x -> x
...
normalizePhone n = valLength <* valChars
  where
    stripped = filter (not . isSpace) n
    valLength = check (length stripped <= 10) "Too long" stripped
    valChars = traverse (\x -> check (isDigit x) ("Invalid character: " ++ [x]) x) stripped
