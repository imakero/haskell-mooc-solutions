...
count val = foldr (\x s -> if x == val then s + 1 else s) 0
...
instance Foldable List where
  ...
  foldr f val (LNode x xs) = f x (foldr f val xs)
...
instance Foldable TwoList where
  ...
  foldr f val (TwoNode x y xs) = f x (f y (foldr f val xs))
...
Logger la a #> f =
  let Logger lb b = f a -- feed value to next step
