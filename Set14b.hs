...
deposit db t x = execute db depositQuery (T.unpack t, x)
...
balance db t = do
  res <- query db balanceQuery [T.unpack t] :: IO [[Int]]
  return $ if not (null res) then sum . concat $ res else 0
...
parseCommand ts = case cmdType of
  Just cmd -> parseCommand' cmd ts
  Nothing -> Nothing
  where
    cmdType = safeIndex ts 0

parseCommand' cmd ts
  | cmd == T.pack "balance" = do
    correctLength ts 2
    name <- name
    return $ Balance name
  ...
  where
    name = safeIndex ts 1
    amount = safeIndex ts 2 >>= parseInt
    correctLength xs n
      | length xs == n = Just n
      | otherwise = Nothing
...
perform db (Just (Deposit n x)) = do
  deposit db n x
  return $ T.pack "OK"
perform db (Just (Balance n)) = do
  res <- balance db n
  return $ T.pack (show res)
perform db (Just (Withdraw n x)) = do
  deposit db n (- x)
  return $ T.pack "OK"
