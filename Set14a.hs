...
greetText name
  | T.length name <= 15 = T.pack "Hello, " <> name <> T.pack "!"
  | otherwise = T.pack "Hello, " <> T.take 15 name <> T.pack "...!"
...
shout :: T.Text -> T.Text
shout s = T.unwords (go $ T.words s)
  where
    go [] = []
    go [w] = [T.toUpper w]
    go (w : x : ws) = T.toUpper w : x : go ws
...
longestRepeat t = maximum . map T.length $ group t
  where
    group s = case T.uncons s of
      Nothing -> [T.pack ""]
      Just (x, rest) -> T.takeWhile (== x) s : group (T.dropWhile (== x) s)
...
pingpong bs = BL.fromStrict bs <> BL.reverse (BL.fromStrict bs) <> pingpong bs