...
--  Picture (\(Coord x y) -> if contains shape x y then color else f (Coord x y))
...
instance Transform Blur where
  apply Blur (Picture f) = Picture g
    where
      g (Coord x y) = avgColor [f (Coord x y), f (Coord (x -1) y), f (Coord (x + 1) y), f (Coord x (y -1)), f (Coord x (y + 1))]

avgColor cs = Color (getRed (sumColors cs) `div` n) (getGreen (sumColors cs) `div` n) (getBlue (sumColors cs) `div` n)
  where
    n = length cs

sumColors xs = foldr (\(Color r1 g1 b1) (Color r2 g2 b2) -> Color (r1 + r2) (g1 + g2) (b1 + b2)) (Color 0 0 0) xs
