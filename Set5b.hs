...
treeMax Empty = 0
treeMax (Node n t1 t2) = max n $ max (treeMax t1) (treeMax t2)
...
allValues condition (Node n t1 t2) = condition n && allValues condition t1 && allValues condition t2
...
cull val (Node n t1 t2)
  | n == val = Empty
  | otherwise = Node n (cull val t1) (cull val t2)
...
walk (StepL : ss) (Node n t1 t2) = walk ss t1
walk (StepR : ss) (Node n t1 t2) = walk ss t2
...
set [] v (Node _n left right) = Node v left right
set (StepL : ss) v (Node n left right) = Node n (set ss v left) right
set (StepR : ss) v (Node n left right) = Node n left (set ss v right)
set _ _v Empty = Empty
...
search v = go v []
  where
    go v path Empty = Nothing
    go v path (Node n left right)
      | v == n = Just path
      | isNothing (go v (path ++ [StepL]) left) = go v (path ++ [StepR]) right
      | otherwise = go v (path ++ [StepL]) left