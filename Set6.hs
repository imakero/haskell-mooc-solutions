...
instance Price a => Price [a] where
  price as = foldr (\a acc -> acc + price a) 0 as
...
instance Ord Number where
  ...
  (<=) Infinite (Finite m) = False
  (<=) (Finite m) Infinite = True
...
instance Eq RationalNumber where
  (RationalNumber pn pd) == (RationalNumber qn qd) = pn * qd == pd * qn
...
instance Num RationalNumber where
  ...
  signum (RationalNumber n d) = RationalNumber (div n (abs n)) (div d (abs d))
