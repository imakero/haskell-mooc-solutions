...
prettyPrint size = foldr insertQueen (empty size)
  where
    empty n = concat . replicate n $ replicate n '.' ++ "\n"
    insertQueen (r, c) board = take index board ++ "Q" ++ drop (index + 1) board
      where
        index = (r -1) * (size + 1) + (c -1)
...
danger c = any threat
  where
    threat q = sameRow c q || sameCol c q || sameDiag c q || sameAntidiag c q
...
prettyPrint2 n qs = [tile r c | r <- [1 .. n], c <- [1 .. n + 1]]
  where
    tile r c
      | c > n = '\n'
      | (r, c) `elem` qs = 'Q'
