...
sumSuccess xs = if s == 0 && not (any rights xs) then Left "no data" else Right s
  where
    s = foldr addEither 0 xs
    addEither (Right x) y = x + y
    addEither (Left x) y = y
    rights (Right x) = True
    rights _ = False
...
-- compose xs ys = foldr composing [] xs
--   where
--     composing (k, v) acc =
--       case lookup v ys of
--         (Just v') -> (k, v') : acc
--         Nothing -> acc
compose xs ys = concatMap apply xs
  where
    apply (k, v) =
      case lookup v ys of
        Nothing -> []
...
permute p xs = map snd $ sort (zip p xs)
